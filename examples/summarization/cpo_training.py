...
from trl.trainer.cpo_trainer import compute_dpo_metrics
from trl.trainer.utils import pad_to_length

"""
TODO:
1. 
    a. build the attention mask for pi (x, y_\theta)
    b. rewrite the attention mask (x, y_w/y_l)
    IMPORTANT: bug, get the (x, y) next token prediction rather than (x,)
2. set the tricks: 
    a. / torch.sqrt() similar to attention mask 
    b. set the utd 
    c. KL divergence (optimal)
    d. set the reward function
"""
...
class ScriptArguments:
    ...
    beta: Optional[float] = field(default=0.1, metadata={"help": "the beta parameter for CPO loss"})
    ...
    adapters: Optional[List[str]] = field(default_factory=lambda: ['policy', 'rep'], metadata={"help": "the adapters to train"})
...
# TODO: add a function to compute gold reward in cpo
...
def create_and_prepare_model(args):
    ...
    if args.use_peft:
        ...
        model.add_adapter(peft_config, adapter_name='policy') # policy model
        model.add_adapter(peft_config, adapter_name='rep') # rep model
    ...
    def on_evaluate(self, args, state, control, model, tokenizer, metrics, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if samples_to_log:
                ...
                csv_file_path = f"results/cpo_{script_args.beta}/game_log_epoch_{state.epoch}.csv"
...
if __name__ == "__main__":
    ...
    cpo_trainer.add_callback(gold_eval_callback)

    # 6. train
    if not script_args.just_eval:
        last_checkpoint = get_last_checkpoint(script_args.output_dir)
        # model.set_adapter(['policy', 'rep'])
        [mod.requires_grad_(True) for n, mod in cpo_trainer.model.named_modules() if "lora" in n]
        cpo_trainer.train(resume_from_checkpoint=last_checkpoint)
        save_path = f"models/summarization/cpo_beta_{script_args.beta}"
        cpo_trainer.save_model(output_dir=save_path)
        print(f"save model to: {save_path}")
    else:
        cpo_trainer.evaluate()
